...
namespace MvcCoreHospitalesBackFront.Services
{
    public class ServiceApiHospital
    {
        ...
        {
            //LAS URL DE APIS EN LOS SERVICES SOLO SE INCLUYE EL SITIO WEB,
            //NO LA PETICION (NO api/hospitales)
            this.UrlApi = "https://apihospitalesazure2023ntc.azurewebsites.net";
        ...
        public async Task<List<Hospital>> GetHospitalesAsync()
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                //REALIZAMOS UNA PETICION ASINCRONA CON EL METODO GET Y NOS DEVOLVERA UNA RESPUESTA DE 
                //TIPO HttpResponseMessage
                HttpResponseMessage response = await client.GetAsync(request);
                //COMPROBAMOS SI LA RESPUESTA ES CORRECTA
                if(response.IsSuccessStatusCode)
                {
                    //AQUI TENEMOS LOS DATOS EN LA PROPIEDAD Content DE LA RESPUESTA VIENEN LOS DATOS 
                    //EN FORMATO JSON
                    //DEBEMOS CONVERTIR LOS DATOS A CLASES MEDIANTE UN METODO LLAMADO ReadAsync()
                ...
                {
                    //ALGO HA FALLADO, SIEMPRE QUE ALGO FALLA, DEVOLVEREMOS NULL DESDE EL SERVICIO
                    return null;
                }
